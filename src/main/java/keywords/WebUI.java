...
public class WebUI {
      ...
      public static void addCookie(String name, String value){
            ...
            logConsole("Navigate with cookie " +value);
      }
      ...
      public static void checkContainsValueOnTableByColumn(int column, String checkValue) {
            List<WebElement> totalRows = getWebElements(By.xpath("//tbody/tr"));
            logConsole("Number of results for keywords (" + checkValue + "): " + totalRows.size());

            if (totalRows.size() < 1) {
                  logConsole("Not found value: " + checkValue);
            } else {
...
//                        sleep(2);
                        res = title.getText().toUpperCase().contains(checkValue.toUpperCase());
                        logConsole("Row " + i + ": " + title.getText());
                        Assert.assertTrue(res, "Row " + i + " (" + title.getText() + ")" + "NOT contains the search value: " + checkValue);
                  }
            }
      }
...
//      public void selectValueFromDynamicDropdown(By searchBoxLocator, By optionLocator, String valueToSelect) {
...
//                  searchBox.click();
//
//                  // Type the value to search in the dropdown
//                  searchBox.clear();  // Clear the field if it's already populated
...
//      }
      ...
      public static void selectValueFromDynamicDropdown(By searchBoxLocator, By optionLocator, String valueToSelect) {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(10));
            ...
            // Wait for the options to be visible (ensure the options are updated after typing)
            List<WebElement> options = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(optionLocator));
